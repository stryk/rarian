...
CKEDITOR.plugins.add('wordcount', {
    ...
    init: function (editor) {
        ...
        if (config.showCharCount) {
            var charLabel = config.countHTML ? CharCountWithHTML : CharCount;

            defaultFormat += charLabel + '&nbsp;%charCount%';

            if (config.charLimit != 'unlimited') {
                defaultFormat += '&nbsp;(' + limit + '&nbsp;' + config.charLimit + ')';
            }
        }
        ...
        if (config.showWordCount) {
            ...
            if (config.wordLimit != 'unlimited') {
                defaultFormat += '&nbsp;(' + limit + '&nbsp;' + config.wordLimit + ')';
            }
        }
        ...
        editor.on('uiSpace', function (event) {
            if (event.data.space == 'bottom') {
                event.data.html += '<div id="' + counterId(event.editor) + '" class="cke_wordcount" style=""' + ' title="' + 'Word' + '"' + '>&nbsp;</div>';
            }
        }, editor, null, 100);
