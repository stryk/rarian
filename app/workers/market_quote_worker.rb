class MarketQuoteWorker
	...
	recurrence do
		minute_hand = []
		0.step(50, 20) {|num| minute_hand << num}
		weekly(1).day(1,2,3,4,5).hour_of_day(9, 10, 11, 12, 13, 14, 15, 16, 17).minute_of_hour(minute_hand)
	end 
	def perform
		if configatron.AUTO_QUOTE_UPDATE == true
			5.times do
				...
				companies_url = ""
				batch = 100
				companies = Group.find_by_name('S&P 500').companies.order(:quote_updated_at => :asc).limit(batch)
			...
			4.times do
				...
				companies_url = ""
				batch = 100
				companies = Group.find_by_name('S&P 400').companies.order(:quote_updated_at => :asc).limit(batch)
				companies.each do |company|
					companies_url = companies_url + "%22" + company.ticker.gsub(/\^/, "-") + "%22" + url_and
				end
				companies_url = companies_url[0..-4]
			...
			6.times do
				...
				companies_url = ""
				batch = 100
				companies = Group.find_by_name('S&P 600').companies.order(:quote_updated_at => :asc).limit(batch)
				companies.each do |company|
					companies_url = companies_url + "%22" + company.ticker.gsub(/\^/, "-") + "%22" + url_and
				end
				companies_url = companies_url[0..-4]
