class Quote < ActiveRecord::Base
  ...
  def self.get_cap(params)
  	...
  	if params[:get] && params[:get] == 'small_cap'
  		company_ids = Quote.where('market_cap < 1 AND created_at > DATE(?)', Time.now - 7.days).order('company_id DESC').
      ...
      company_ids = Quote.where('market_cap >= 1 and market_cap <= 10 AND created_at > DATE(?)', Time.now - 7.days).order('company_id DESC').
      group('company_id').select('count(quotes.id), quotes.company_id').map(&:company_id)
      return_value = [company_ids, 'get']
    elsif params[:get] && params[:get] == 'large_cap'
      company_ids = Quote.where('market_cap > 10 AND created_at > DATE(?)', Time.now - 7.days).order('company_id DESC').
      group('company_id').select('count(quotes.id), quotes.company_id').map(&:company_id)
      return_value = [company_ids, 'get']
    elsif params[:sector]
